...
const generateBossImage = async (prompt: string): Promise<string | null> => {
    try {
        ...
        const imagePath = path.join(__dirname, '..', 'public', 'boss-images', fileName);
        const writer = fs.createWriteStream(imagePath);

        return new Promise<string | null>((resolve, reject) => {
            imageResponse.data.pipe(writer);
            writer.on('finish', () => {
                resolve(`/boss-images/${fileName}`);
            });
            writer.on('error', (err) => {
                reject(err);
            });
...
const getBoss = async (req: Request, res: Response) => {
...
const attackBoss = async (req: Request, res: Response) => {
    const userId = (req as any).user.userId;
    ...
    try {
        ...
        if (updatedHealth <= 0) {
            ...
            const levelUp = updatedExperience >= 10 * (character.level ** 2);
...
const createNewBoss = async (req: Request, res: Response) => {
    const userId = (req as any).user.userId;
